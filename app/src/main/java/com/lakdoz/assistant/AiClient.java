...
public class AiClient {
    ...
    private static final class ModelCandidate {
        ...
        final int score;
        ModelCandidate(String name, boolean stream, int score) {
            this.name = name;
            this.stream = stream;
            this.score = score;
        }
    ...
    public String askStreaming(String prompt, List<HistoryStore.Turn> history, DeltaListener listener) throws Exception {
        ...
        if (!cached.isEmpty()) {
            try {
                String answer = streamModel(key, cached, prompt, history, listener);
                settings.setGeminiModel(cached);
                return answer;
            } catch (Exception e) {
                last = e;
        ...
        if (available.isEmpty()) {
            ...
            throw new IllegalStateException("Bu API anahtarı için uygun Gemini metin modeli bulunamadı.");
        }

        int tried = 0;
        for (ModelCandidate candidate : available) {
            if (candidate.name.equals(cached)) continue;
            if (tried >= 3) break;
            tried++;
            try {
                String answer = candidate.stream
                        ? streamModel(key, candidate.name, prompt, history, listener)
                        : generateModel(key, candidate.name, prompt, history, listener);
                settings.setGeminiModel(candidate.name);
                return answer;
            } catch (Exception e) {
                last = e;
                if (!canFallback(e)) throw e;
    ...
    private List<ModelCandidate> discoverModels(String key) throws Exception {
        ...
        if (code < 200 || code >= 300) throw httpError(c, code, "models.list");

        JSONObject root = new JSONObject(readAll(c.getInputStream()));
        JSONArray models = root.optJSONArray("models");
        ArrayList<ModelCandidate> out = new ArrayList<>();
        if (models == null) return out;

        for (int i = 0; i < models.length(); i++) {
            JSONObject m = models.optJSONObject(i);
            if (m == null) continue;
            String name = cleanModelName(m.optString("name", ""));
            if (name.isEmpty() || !name.toLowerCase().contains("gemini")) continue;
            String lower = name.toLowerCase();
            if (lower.contains("tts") || lower.contains("image") || lower.contains("embedding") ||
                    lower.contains("imagen") || lower.contains("veo") || lower.contains("robot") ||
                    lower.contains("live") || lower.contains("audio") || lower.contains("computer-use")) continue;
            ...
            if (methods != null) {
                for (int j = 0; j < methods.length(); j++) {
                    ...
                    if ("generateContent".equals(method)) generate = true;
                    if ("streamGenerateContent".equals(method)) stream = true;
                }
            }
            if (!generate && !stream) continue;

            int score = 100;
            if (lower.contains("flash-lite")) score += 1000;
            else if (lower.contains("flash")) score += 850;
            else if (lower.contains("pro")) score += 350;
            if (lower.contains("latest")) score += 90;
            if (stream) score += 70;
            if (lower.contains("preview")) score -= 40;
            out.add(new ModelCandidate(name, stream, score));
        }

        Collections.sort(out, new Comparator<ModelCandidate>() {
            @Override public int compare(ModelCandidate a, ModelCandidate b) {
                int byScore = Integer.compare(b.score, a.score);
                return byScore != 0 ? byScore : b.name.compareTo(a.name);
            }
        });
        return out;
    }
    ...
    private String streamModel(String key, String model, String prompt, List<HistoryStore.Turn> history, DeltaListener listener) throws Exception {
        ...
        HttpURLConnection c = openPost(endpoint, body);
        int code = c.getResponseCode();
    ...
    private String generateModel(String key, String model, String prompt, List<HistoryStore.Turn> history, DeltaListener listener) throws Exception {
        ...
        int code = c.getResponseCode();
        if (code < 200 || code >= 300) throw httpError(c, code, model);
        String text = extractText(new JSONObject(readAll(c.getInputStream()))).trim();
        if (text.isEmpty()) throw new IllegalStateException("Gemini boş cevap döndürdü.");
        if (listener != null) listener.onDelta(text);
        return text;
    }

    private HttpURLConnection openPost(String endpoint, JSONObject body) throws Exception {
        HttpURLConnection c = (HttpURLConnection) new URL(endpoint).openConnection();
    ...
    private JSONObject buildBody(String prompt, List<HistoryStore.Turn> history) throws Exception {
        ...
        for (int i = start; i < history.size(); i++) {
            ...
            item.put("role", "assistant".equals(t.role) ? "model" : "user");
            item.put("parts", new JSONArray().put(new JSONObject().put("text", t.text)));
            contents.put(item);
        }
    ...
    private boolean canFallback(Exception e) {
        if (e instanceof SocketTimeoutException) return true;
        String m = e.getMessage() == null ? "" : e.getMessage().toLowerCase();
        return m.contains("http 404") || m.contains("http 408") || m.contains("http 429") ||
                m.contains("http 500") || m.contains("http 502") || m.contains("http 503") ||
                m.contains("http 504") || m.contains("timed out") || m.contains("timeout");
    }
    ...
    private Exception httpError(HttpURLConnection c, int code, String model) throws Exception {
        String msg = readAll(c.getErrorStream());
        try {
