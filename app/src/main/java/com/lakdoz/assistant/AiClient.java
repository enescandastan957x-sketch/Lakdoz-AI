...
public class AiClient {
    ...
    private JSONObject buildBody(String prompt,List<HistoryStore.Turn> history,String memory,boolean grounded)throws Exception{
        JSONObject body=new JSONObject();StringBuilder sys=new StringBuilder("Sen Lakdoz AI adlı hızlı, bağlamı takip eden Türkçe kişisel asistansın. Önce doğrudan cevabı ver. Kısa sorulara kısa cevap ver; kullanıcı ayrıntı isterse ayrıntılandır. Kullanıcının yazım hatalarını bağlamdan makul biçimde düzelt. Takip sorularında önceki mesajdaki kişi, yer, konu ve zamanı koru; gereksiz yere tekrar sorma. Bilmediğin veya güncel olmayan şeyi uydurma. Canlı bir araçtan doğrulanmış veri geldiyse onu önceliklendir. Telefon komutları Android araçları tarafından ayrıca işlenir.");
