...
public class AiClient {
    ...
    public String askStreaming(String prompt,List<HistoryStore.Turn> history,String memory,DeltaListener listener)throws Exception{
        String key=settings.getGeminiApiKey(); if(key==null||key.trim().isEmpty())throw new IllegalStateException("Gemini bağlantısı ayarlı değil. Ayarlar bölümünden anahtarını ekle."); key=key.trim();
        ...
        List<ModelCandidate> available=discoverModels(key); if(available.isEmpty()){if(last!=null)throw last;throw new IllegalStateException("Bu API anahtarı için uygun Gemini metin modeli bulunamadı.");}
    ...
    private JSONObject buildBody(String prompt,List<HistoryStore.Turn> history,String memory)throws Exception{
        JSONObject body=new JSONObject();StringBuilder sys=new StringBuilder("Sen Lakdoz adlı hızlı Türkçe kişisel yapay zekâ asistansın. Önce doğrudan cevabı ver. Kısa sorulara kısa cevap ver; kullanıcı ayrıntı isterse ayrıntılandır. Doğal ve samimi konuş. Bilmediğin şeyi uydurma. Telefon komutları Android araçları tarafından ayrıca işlenir.");
        ...
        body.put("systemInstruction",new JSONObject().put("parts",new JSONArray().put(new JSONObject().put("text",sys.toString()))));JSONArray contents=new JSONArray();int start=Math.max(0,history.size()-8);for(int i=start;i<history.size();i++){HistoryStore.Turn t=history.get(i);contents.put(new JSONObject().put("role","assistant".equals(t.role)?"model":"user").put("parts",new JSONArray().put(new JSONObject().put("text",t.text))));}contents.put(new JSONObject().put("role","user").put("parts",new JSONArray().put(new JSONObject().put("text",prompt))));body.put("contents",contents);body.put("generationConfig",new JSONObject().put("maxOutputTokens",800).put("temperature",0.55));return body;
    }
